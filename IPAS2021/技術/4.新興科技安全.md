...
- I1 弱密碼、可猜測密碼或裝置預設密碼[Weak Guessable, or Hardcoded Passwords](https://www.opencli.com/internet/router-common-login-password)
  - 密碼簡單容易破解、或硬體預設密碼。
- I2 不安全的網路服務 Insecure Network Services 不安全的網路服務
  - 使用未加密的網路協議
- I3 不安全的網路設定介面 Insecure Ecosystem Interfaces
  - 設備本身執行不必要或不安全的網路服務（如 Telnet、FTP 該服務為未加密的服務），將造成敏感資料外洩影響設備的機密性、完整性、可用性，甚至有機會造成未經授權的遠端指令執行。
- I4 缺乏安全更新設備的機制 Lack of Secure Update Mechanism
  - 設備可能無安全更新的機制，若發生弱點因無更新機制，導致設備易遭受到惡意攻擊者侵入，安全更新機制亦包含驗證更新韌體檔合法性（未被惡意攻擊者竄改過）、韌體傳輸過程未加密、設備中防止韌體更新非最新版本的功能以及缺少版本更新後通知使用者的功能。
- I5 使用不安全或已遭棄用的元件 Use of Insecure or Outdated Components
...
- I7 不安全的資料傳輸與儲存 Insecure Data Transfer and Storage [不安全的資料儲存](https://www.ithome.com.tw/news/145196)
