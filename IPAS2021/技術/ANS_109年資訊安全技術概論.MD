...
(D) 開放連接埠 22，UDP 連線
```
[很多protocol的port](https://dotblogs.com.tw/ghoseliang/2013/02/20/91159)
> - FTP 20\21 port
...
```
> - 連線劫持都劫持怎麼會入侵 <- [中間人攻擊實現](https://zh.wikipedia.org/wiki/%E4%BC%9A%E8%AF%9D%E5%8A%AB%E6%8C%81)
```
6.網路`應用`程式防火牆（Web Application Firewall）
...
(A) SSL 1.2
(B) SSL 3.0
(C) TLS 1.2 <- TLS是SSL改良版 SSL已經被破了
(D) TLS 3.0 沒有3.0 最大1.3
...
(D) 使用圖形驗證碼 <-
```
跨站請求偽造（Cross-Site Request Forgery, CSRF） <- 圖形驗證碼
```
...
或是攻擊者是否需要登入才能利用此弱點漏洞利用程度、修復級別與漏洞嚴重程度`評分`等資訊，
請問他應該檢視漏洞報告內的何種指標資訊？
(A) CSV
(B) NVD
(C) VSS
...
(D) 跨站指令碼攻擊（Cross-Site Scripting, XSS）<- Cookie
